...
static const char *aliases[][MAX_NUM_ALIASES] = {
	{"Officer Keypie", "Offier Keypie", NULL}
};

static time_t time_constraint;
static struct rb_root client_root = RB_ROOT;
...
static __hot void log_conn1(const char *client_name, int id, time_t t)
{
	struct rb_node **p = &client_root.rb_node;
	struct rb_node *parent = NULL;
	struct client *c;

	while (*p) {
		parent = *p;
		c = rb_entry(parent, struct client, rb_node);
	...
	rb_insert_color(&c->rb_node, &client_root);
found:
	if (++c->num_conn == 1) {
		c->last_time_connected = t;
		if (strncmp(c->name, client_name, MAX_CLIENT_NAME)) {
...
static __hot void log_disconn1(int id, time_t t_disconn)
{
	struct rb_node **p = &client_root.rb_node;
	struct rb_node *parent = NULL;
	struct client *c;
	...
	if (!*p)
		return;
	c = rb_entry(*p, struct client, rb_node);
	if (c->num_conn) {
		if (c->num_conn == 1 && c->last_time_connected) {
			c->total_time_connected +=
				t_disconn - c->last_time_connected;
			c->last_time_connected = 0;
		}
...
static void free_tree(struct rb_root *root)
{
	while (!RB_EMPTY_ROOT(root)) {
		...
		rb_erase(p, root);
		free(rb_entry(p, struct client, rb_node));
	}
}
...
{
	struct rb_node *node;
	struct rb_node *(*start_func)(const struct rb_root *);
	struct rb_node *(*iter_func)(const struct rb_node *);
	unsigned long count;

	/*
	 * Tree traversal operations depend on if we want the head or tail
	 * of the list.
	 */
	if (head_count) {
		start_func = rb_last;
		...
		/* Default (no arguments given) case */
		start_func = rb_first;
		iter_func = rb_next;
		count = ~0UL;
	}
	...
	/* Iterate through the tree and print all the clients */
	for (node = start_func(root); node && count; count--, node = iter_func(node)) {
		struct client *c = rb_entry(node, struct client, rb_node);
		printf("%lu\t%s\n", c->total_time_connected, c->name);
	}
}

static void sort_and_print_tree(void)
{
	struct rb_root sort = RB_ROOT;

	/*
	 * Transfer all of our clients out of the id tree and into
	 * a new tree that is arranged by time_connected.
	 */
	while (!RB_EMPTY_ROOT(&client_root)) {
		struct rb_node **q = &sort.rb_node;
		struct rb_node *parent = NULL, *p = client_root.rb_node;
		struct client *c, *k;
		time_t t;

		/* Remove the client from the id tree */
		rb_erase(p, &client_root);
		k = rb_entry(p, struct client, rb_node);
		t = k->total_time_connected;

		/*
		 * Client we removed will now be inserted into a new tree
		 * sorted by time.
		 */
		while (*q) {
			parent = *q;
			c = rb_entry(parent, struct client, rb_node);
			if (t < c->total_time_connected)
				q = &(*q)->rb_left;
			else
				q = &(*q)->rb_right;
		}
		rb_link_node(&k->rb_node, parent, q);
		rb_insert_color(&k->rb_node, &sort);
	}
	print_tree(&sort);
	free_tree(&sort);
}
...
#else
	for (node = rb_first(&client_root); node; node = rb_next(node)) {
		c = rb_entry(node, struct client, rb_node);
		c->num_conn = 0;
		c->last_time_connected = 0;
	}
#endif
